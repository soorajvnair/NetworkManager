...
rebuild_domain_lists (NMDnsManager *self)
{
	...
	c_list_for_each_entry (ip_data, _ip_config_lst_head (self), ip_config_lst) {
		 ...
		 * If there is no default route, add the wilcard domain to all non VPN
		 * connections */
		...
		for (i = 0; i < n_domains; i++) {
			...
			if (domain_is_shadowed (ht, domain_clean, priority, &parent, &parent_priority)) {
			...
			_LOGE ("plugin: add domain '%s' (i=%d, p=%d)", domains[i], ip_data->data->ifindex, priority);
