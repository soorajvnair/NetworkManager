                      ...
                      NMDnsIPConfigData *data)
{
	...
	nm_assert (data->domains.search);

	if (!data->domains.search[0])
		return;
	...
	for (iter = data->domains.search; *iter; iter++) {
		...
		g_variant_builder_add (domains, "(sb)", domain, is_routing);
	}
}
...
send_updates (NMDnsSystemdResolved *self)
{
	...
	c_list_for_each_entry_safe (request_item,
	                            ...
	                            &priv->request_queue_lst_head,
	                            request_queue_lst) {
		_LOGE ("send %s (%s)", request_item->operation, g_variant_print (request_item->argument, FALSE));
